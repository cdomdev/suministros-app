...
export const CardProductos: React.FC<Producto> = ({ discount, id, image, marca, nombre, referencia, valor }) => {
    return (
        <a
        ...
        >
            <div className="flex w-full justify-between items-center mb-1">
                ...
                <span className="text-gray-700 text-[8px] md:text-[10px] uppercase">REF: {referencia}</span>
            </div>
            ...
            <div className="mt-2 flex flex-col justify-between gap-2 flex-grow">
                <ul className="flex flex-col justify-center items-center leading-3">
                    <li className="text-gray-400 text-xs font-semibold mt-3 uppercase">{marca}</li>
                    <li className="text-xs md:text-sm leading-5 text-center">{nombre}</li>
                    {discount ? (
                        <>
                            ...
                            <li className="text-sm md:text-lg my-2 font-semibold">
                                $ {calcularDescuento(valor, discount)}
                            </li>
                        </>
                    ) : (
                        <li className="text-sm md:text-lg font-semibold mt-4">$ {formateValue(valor)}</li>
                    )}
                </ul>
                <div className="flex justify-center w-full">
                    <button
                        className="bg-blue-700 border-none py-2 text-[10px] max-w-[60%] mt-2 text-white rounded-sm hover:bg-blue-500 duration-100 md:text-sm w-full"
                    >
                        Ver producto
                    </button>
                </div>
