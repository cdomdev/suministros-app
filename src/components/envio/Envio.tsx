...
export default function Envio() {
  ...
  const handleSubmit = async (values: DatosUsurio) => {
    ...
    setDepartamento(values.departamento || "No hay dato");
    setCiudad(values.ciudad || "no hay dato");
  ...
  return (
    <>
      <button
        ...
        className="bg-blue-600 w-full md:w-[30%] mx-auto  p-2 rounded-md text-white hover:bg-blue-500 duration-200 text-xs md:text-sm"
      >
      ...
      <Modal isOpen={show} onClose={() => setShow(false)}>
        <Formik
          ...
          validate={(values: DatosUsurio) => {
            ...
            if (!values.direccion) {
              errors.direccion = "*Debe seleccionar una opcion*";
            }
            if (!values.telefono) {
